...
class TaskAttention(nn.Module):
    def __init__(self, num_tasks):
        ...
        self.attention_module = nn.Sequential(
             ...
             nn.Conv2d(256, self.num_tasks, kernel_size=1, bias=True),
             nn.Softmax(dim=1)
        )
    ...
    def forward(self, x):
        ...
        for i in range(self.num_tasks):
            a_t_repeat = a_t[i].repeat(1,256,1,1) #expand a_t value to the same dimension with x
            att_feature = a_t_repeat * x
            att_feature = att_feature.view(x.size(0), 256 * 6 * 6)
            att_features.append(self.classifier(att_feature)) #(B, 4096)
        return att_features
...
class ClothingAttributeNet_alexnet(nn.Module):
    def __init__(self, model_cfg):
        super(ClothingAttributeNet_alexnet, self).__init__()
        self.num_classes = model_cfg["NUMNBER_OF_CLASSES"]

        if model_cfg["BACKBONE"] == 'alexnet':
            self.backbone = alexnet(True)

        if model_cfg["BACKBONE"] == 'resnet50':
            self.backbone = resnet50(True)

        if model_cfg["BACKBONE"] == 'resnet101':
...
if __name__ == '__main__':
    ...
    model = ClothingAttributeNet_alexnet(model_cfg)
